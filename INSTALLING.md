...
`43f` has been tested on Mac OS X, Linux, and OpenBSD. It requires Bash 3+ and
Sebastian Freundt's [Dateutils](http://www.fresse.org/dateutils/) for
calculating relative dates. These must be installed prior to following the
...
3. Optional, but suggested: run the test suite as follows (requires
    [dateutils](http://www.fresse.org/dateutils/) and
